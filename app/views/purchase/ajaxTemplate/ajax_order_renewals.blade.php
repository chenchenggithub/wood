...
<div class="main-container-title">总购买信息<span class="font-normal">（{{$order_other['package_type']}}：<span class="text-red">￥{{$order_other['package_price']['renewals_unit_price']}}/月</span>）</span></div>
<div class="main-container-body">
    ...
    <div class="cost_block">
        <div class="form-horizontal">
            ...
            <div class="form-group">
                ...
                <div class="col-md-7"><p class="form-control-static">￥{{$order_other['package_price']['renewals_unit_price']}}</p></div>
            </div>
            <div class="form-group">
                <label for="" class="col-md-4 control-label">合计：</label>
                <div class="col-md-7"><p class="form-control-static">￥{{$order_other['package_price']['renewals_unit_price']}} * {{$order_other['package_price']['renewals_time']}}月 = ￥{{$order_other['package_price']['total_price']}}</p></div>
            </div>
            <div class="form-group">
    ...
    <div class="modal-dialog" style="width:680px;">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p class="monitor_des">总的购买监测点数 <span class="text-red">{{$commodity[PackageEnum::MONITOR]['monitor_count']}}</span> 个，总值 <span class="text-red" id="select_yundou_num">{{$commodity[PackageEnum::MONITOR]['yundou']}}</span> 云豆</p>
                <div class="modal-panel-body">
                    <table class="table table-striped">
                        ...
                        <tbody id="load_order_renewal_monitor_list">

                        </tbody>
                    </table>
                    <ul class="pagination pull-right" id="order_renewal_monitor_page_list_id">
...
<script>
    ...
    //加载已选择的监测点
    function loadOrderRenewalMonitorFunc(page){
        var order_renewal_monitor = {{$order_renewal_monitor}};
        var page_num = {{$page_num}};
        var total = tsb_count(order_renewal_monitor);
        var start = (page-1)*page_num+1;
        var end = page*page_num;
        if(end >total) end = total;
        var selected_monitor_list = '';
        var count_num = 1;
        for(var i in order_renewal_monitor){
            if(count_num >= start && count_num<=end)
                selected_monitor_list += '<tr><td>'+order_renewal_monitor[i][1]+'</td><td>'+order_renewal_monitor[i]['area_name']+'</td><td>'+order_renewal_monitor[i][2]+'</td></tr>';
            count_num++;
        }

        $("#load_order_renewal_monitor_list").html(selected_monitor_list);
        var page_html = getOrderRenewalMonitorPageList(page,page_num,total);
        $("#order_renewal_monitor_page_list_id").html(page_html);
    }
    //获取分页列表
    function getOrderRenewalMonitorPageList(page,page_num,total){
        ...
        page_html += '<li><a href="javascript:loadOrderRenewalMonitorFunc('+page_head+');">«</a></li>'
        for(var i=1;i<=all_page;i++){
            if(i == page){
                page_html += '<li class="active"><a href="javascript:loadOrderRenewalMonitorFunc('+i+');">'+i+'</a></li>';
            }else{
                page_html += '<li><a href="javascript:loadOrderRenewalMonitorFunc('+i+');">'+i+'</a></li>';
            }
        }
        page_html += '<li><a href="javascript:loadOrderRenewalMonitorFunc('+page_last+');">»</a></li>'
        return page_html;
    }
    loadOrderRenewalMonitorFunc(1);
